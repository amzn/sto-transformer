...
class IMDB(nn.Module):
    def __init__(self, emb_dim, output_dim, vocab, dropout=0.1):
        ...
        self.transformer_encoder = StoTransformerEncoder(src_vocab_size=self.vocab_size, src_pad_idx=0,
                                                         emb_dim=self.emb_dim, num_layers=args.n_layer,
                                                         forward_expansion=args.n_fe, heads=args.n_head,
                                                         dropout=args.dropout, device=args.device,
                                                         max_length=args.sentence_len,
                                                         k_centroid=args.k_centroid, tau1=args.tau1, tau2=args.tau2,
    ...
    def sentiment_classification_v0(self, query):
        query_enc, query_mask = self.transformers_encode(
            query)  # [batch_size, 1, seq_len] -> [batch_size, 1, seq_len, n_dim]
    ...
    def do_infer(self, data):
        text = data['text']  # .unsqueeze(1)
        label = data['label']
        prediction = F.softmax(self.sentiment_classification(text), dim=-1)  # [batch_size, output_dim]
