...
class IMDB(nn.Module):
    def __init__(self, emb_dim, output_dim, vocab, dropout=0.1):
        ...
        self.transformer_encoder = StoTransformerEncoder(src_vocab_size=self.vocab_size, src_pad_idx=0, emb_dim=self.emb_dim, num_layers=args.n_layer,
                                                         forward_expansion=args.n_fe, heads=args.n_head, dropout=args.dropout, device=args.device, max_length=args.sentence_len,
                                                         k_centroid=args.k_centroid, tau1=args.tau1, tau2=args.tau2, direction=args.direction)
    ...
    def sentiment_classification_v0(self, query):
        ...
        # logits = F.gumbel_softmax(logits, tau=args.tau, hard=False)
        return logits
    ...
    def do_train(self, data):
        ...
        #loss_sel = torch.nn.CrossEntropyLoss(input=prediction, target=target)
